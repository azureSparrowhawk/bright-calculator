...
def isPrime(numberToCheck)->bool:
    ...
    if type(numberToCheck) == float:
        return False
    for possibleFactor in range(2, int((numberToCheck/2)+1)):
...
def primeFactorisation(numberToFactorise):
    ...
    while not isListPrime(startList):#While the list containing the (prime) factors is not completely made of primes
        for number in startList:
            if not isPrime(number):#If i isn't prime, it finds something to divide it by
                for possibleFactor in range(2, int((number/2)+1)):
                    if number%possibleFactor == 0:
                        startList.remove(number)
                        startList.append(possibleFactor)
                        startList.append(int(number/possibleFactor))
                        break
    startList.sort()
    return startList
...
def multiples(number, maximumNumber):
    ...
    numbList = [number * multiplier for multiplier in range(1, maximumNumber + 1)]
    numbList.sort()
    return numbList
...
def lcmAndHCF(lastNum, allNumbers):
    ...
    for commonPrimeFactor in commonPrimeFactors:
        HCF *= commonPrimeFactor #Finds HCF by multiplying all the common prime factors together
    LCM = HCF
    for factorList in allFactorLists:
